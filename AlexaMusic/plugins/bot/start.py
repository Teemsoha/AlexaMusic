...
@app.on_message(
    filters.command(["/start","بوتت"],"") & filters.private & ~BANNED_USERS
)
@LanguageStart
...
@app.on_message(
    filters.command("START_COMMAND") & filters.group & ~BANNED_USERS
)
@LanguageStart
...
async def alive(client, message: Message):
    await message.reply_photo(
        photo=f"https://telegra.ph/file/47cea7a75abd4b8a45c4c.mp4",
...
@app.on_message(commandpro(["/dina", "alexaverification"]))
